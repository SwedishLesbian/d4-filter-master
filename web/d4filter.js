...
// ---- rule assembly (mirror build() + build_filter_code) ------------------
export function buildFilter(extracted, name, bundle, opts = {}) {
  const { rows, uniques, charmHashes, hasSeal, cls } = extracted;
  ...
  // per-tier colours, overridable from the UI (ARGB ints); default to the
  // constants above so default output stays byte-identical to the Python oracle.
  ...
  // group desired affixes per slot family, first-appearance order
  ...
  const sealType = bundle.sealType, charmType = bundle.charmType;
  const allTypeIds = bundle.allItemTypeIds || [];
  ...
  function slotConds(sr, n, ga) {
    ...
    conds.push(cAffixes(sr.ids, n, ga));
    return conds;
  }
